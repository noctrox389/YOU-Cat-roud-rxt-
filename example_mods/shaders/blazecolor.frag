...
void mainImage() {
    ...
    const vec3 c2 = vec3(0.831,0.,1.);
    const vec3 c3 = vec3(0.2, 0.1, 0.7);
    const vec3 c4 = vec3(1.0, 0.9, 0.1);
    const vec3 c5 = vec3(0.1);
    const vec3 c6 = vec3(0.9);

    vec2 speed = vec2(5.2, 0.1);
    float shift = 1.327+sin(iTime*10.0)/2.4;
    float alpha = 0.32;
    ...
    color = hsv2rgb(hsv);
    fragColor = vec4(color.x, color.y, color.z, alpha) + texture(iChannel0,uv);
}