...
uniform float deez = 1.0;
uniform float bruh = 2.75;
...
vec3 rayDirection(float fieldOfView, vec2 fragCoord) {
    ...
    float z = (0.5 * iResolution.y) / tan(radians(fieldOfView) / bruh);
...
vec3 rotate(vec3 p, vec4 q){
  return deez * cross(q.xyz, p * q.w + cross(q.xyz, p)) + p;
}
...
void mainImage(){
    ...
    vec3 glowColour = vec3(0.9, 0.3, 0.4);
    ...
    fragColor = vec4(col, 0.0) + texture(iChannel0,uv);
}